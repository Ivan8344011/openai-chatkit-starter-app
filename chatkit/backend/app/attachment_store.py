...
class LocalAttachmentStore(AttachmentStore[dict[str, Any]]):
    ...
    return attachmentasync def delete_attachment(
    self,
    attachment_id: str,
    context: dict[str, Any],
) -> None:
    ...
    attachment = await self._load_attachment_or_404(
        attachment_id,
        context,
    )
    ...
    await self.store.save_attachment(
        attachment,
        context=context,
    )
 ...
 def _validate_size(self, size: int) -> None:
    ...
    try:
        return await self.store.load_attachment(
            attachment_id,
            context=context,
        )
